...
**Scenarios**: relay tests as recipes. Each JSON file in `scenarios/` publishes a small flow as the demo identities and states what every relay should do. Scenarios… in the Technical toolbar runs one and draws the matrix live, one row per step and one column per relay, with every check spelled out and a Markdown report. Four ship with the test bed. Write your own in the window (New Scenario… starts from a recipe, Record turns what you publish into steps, Add to Scenario… takes any loaded event; an editor with the JSON beside the form and identities minted inline) or drop a file into `scenarios/`; either way it appears without a rebuild (`scenarios/README.md` has the format).
